...
int formatCommand(char *str){
    int i,j;
    for(i = 0; str[i] != '\0'; i++){
        if(str[i] == 39){   // ' na tablea ascii
            for(j = i; str[j] != 39; j++){ //O que está entre aspas simples será os caminhos das pastas, não pode ser formatado
                if(str[j] == '\0')
        ...
        if(str[i] == 39){
            ...
            if(arg1[0] != '\0') //verifica se o primeiro já foi atribuido
                for(j = i; str[j] != 39;j++){
                    ...
                    for(j = i; str[j] != 39;j++){
                        arg2[u] = str[j];
                    } else break; //se não existe o segundo, nao tem pq continuar o loop
...
void Donut(){ //nao testei ainda, cuidado
    float A = 0, B = 0;
    float i, j;
    int k;
    float z[1760];
